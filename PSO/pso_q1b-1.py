...
# build gif
with imageio.get_writer('Q1b-cost-1.gif', mode='I') as writer:
...
cities_status[1][:] =  ['grey', 'green', 'green', 'red', 'yellow', 'yellow', 'grey', 'green', 'green', 'grey']
cities_status[0][:] = cities_status[0][:] + ['Ahmedabad'] 

''' Visulizing the path with iteration ''' 
#Assigning random city coordinates. To represent cities as vertices of graph.
cities_coordinates = [[1,63,40,55,19,69,10,36,77,2],[1,1,6,69,35,19,80,46,53, 51]]
fig, ax = plt.subplots()
...
# build gif
with imageio.get_writer('Q1b-Path-1.gif', mode='I') as writer:
    for filename in filenames:
        image = imageio.imread(filename)
        writer.append_data(image)

...
while(current_total_particles<tot_particles):
    try:
        ...
        for x in range(total_cities-1):
            ...
            curr=next
        ## Adding Ahmedabad city randomly two times
        ind=random.randint(0,len(temp_sol))
        temp_sol = temp_sol[:ind] + [0] + temp_sol[ind:]
        ind=random.randint(0,len(temp_sol))
        temp_sol = temp_sol[:ind] + [0] + temp_sol[ind:]
...
total_cities = len(gbest.solution)
...
# build gif
with imageio.get_writer('Q1b-cost-2.gif', mode='I') as writer:
...
cities_status[1][:] =  ['grey', 'green', 'green', 'red', 'yellow', 'yellow', 'grey', 'green', 'green', 'grey','grey']
cities_status[0][:] = cities_status[0][:] + ['Ahmedabad'] + ['Ahmedabad']

''' Visulizing the path with iteration ''' 
#Assigning random city coordinates. To represent cities as vertices of graph.
cities_coordinates = [[1,63,40,55,19,69,10,36,77,2, 29],[1,1,6,69,35,19,80,46,53, 51, 63]]
fig, ax = plt.subplots()
...
filenames = []
for g in Gbest_list:
    #Making the 0 of second ahmedabad as 10th city. Just for simplicity in visulizing the path
    cnt=0
    for f in range(len(g.solution)):
        if(g.solution[f]==0 and cnt==0):
            f=cnt
            cnt=9
            continue
        if(g.solution[f]==0 and cnt==9):
            g.solution[f]=cnt
            cnt=10
            continue
        if(g.solution[f]==0 and cnt==10):
            g.solution[f]=cnt
            continue
    # print(g.solution)
    plt.cla()
...
# build gif
with imageio.get_writer('Q1b-Path-2.gif', mode='I') as writer:
    for filename in filenames:
        image = imageio.imread(filename)
        writer.append_data(image)
...
print("Note - 9,10 means Ahmedbad city.")